...
const oldUrlCount = new Map();
const newUrlCount = new Map();
const missingTitleMetaH1 = [];
...
rows.forEach((row, index) => {
  ...
  if (!row.title || !row.meta_description || !row.h1) missingTitleMetaH1.push(rowNumber);
  if (!row.new_url || !row.redirect_type) missingRedirect.push(rowNumber);
});
...
if (missingColumns.length > 0) console.log(` - ${missingColumns.join(', ')}`);
console.log(`Rows missing title/meta/H1: ${missingTitleMetaH1.length}`);
if (missingTitleMetaH1.length > 0) console.log(` - row numbers: ${missingTitleMetaH1.join(', ')}`);
console.log(`Rows missing redirect target/type: ${missingRedirect.length}`);
if (missingRedirect.length > 0) console.log(` - row numbers: ${missingRedirect.join(', ')}`);
console.log(`Duplicate old URLs: ${duplicateOldUrls.length}`);
if (duplicateOldUrls.length > 0) console.log(` - ${duplicateOldUrls.map(([url]) => url).join(', ')}`);
console.log(`Duplicate new URLs: ${duplicateNewUrls.length}`);
if (duplicateNewUrls.length > 0) console.log(` - ${duplicateNewUrls.map(([url]) => url).join(', ')}`);
