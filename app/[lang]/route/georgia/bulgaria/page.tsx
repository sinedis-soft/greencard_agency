...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  if (!isRouteLocaleIndexable(lang, GEORGIA_BULGARIA_ROUTE)) {
    return { robots: { index: false, follow: false } };
  }

  const t = getGeorgiaBulgariaOcDictionary(lang);
  return { alternates: pageAlternates(lang, GEORGIA_BULGARIA_ROUTE), title: t.seo.title, description: t.seo.description, ...pageSocialMetadata(lang, GEORGIA_BULGARIA_ROUTE, t.seo.title, t.seo.description) };
}
...
export default async function GeorgiaRomaniaOcPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <main id="main">
      ...
      <RouteLandingPage lang={lang} dictionary={t} review={CONTENT_REVIEW} pagePath={GEORGIA_BULGARIA_ROUTE} />
    </main>
  );
}
